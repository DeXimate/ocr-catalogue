from __future__ import annotations

from pathlib import Path
import shutil
import sys

ROOT = Path(__file__).resolve().parent

PANEL = ROOT / "ocr_catalogue" / "offers" / "panel_detector.py"
REGION = ROOT / "ocr_catalogue" / "offers" / "region_solver.py"
RESOLVER = ROOT / "ocr_catalogue" / "offers" / "resolver.py"
CLASSIFIER = ROOT / "ocr_catalogue" / "semantics" / "classifier.py"

FILES = [PANEL, REGION, RESOLVER, CLASSIFIER]


def backup(path: Path) -> None:
    backup_path = path.with_suffix(path.suffix + ".bak_card_first")
    if not backup_path.exists():
        shutil.copy2(path, backup_path)


def replace_once(text: str, old: str, new: str, label: str) -> str:
    count = text.count(old)
    if count != 1:
        raise RuntimeError(f"{label}: motif attendu 1 fois, trouvé {count} fois")
    return text.replace(old, new, 1)


PANEL_CONTENT = r'''from __future__ import annotations

"""Card-first detection for structured catalogue pages.

A structured catalogue page already contains the commercial card geometry in
the PDF. On such pages the card is the source of truth for the crop and for
object ownership. Semantic/OCR geometry is only a fallback for free layouts.

Important invariants:

* only PDF vector CONTAINER objects can become native cards;
* PDF IMAGE objects are never used as card boundaries;
* a structured page is discovered from repeated container geometry;
* a native card must contain exactly one PRICE_MAIN centre;
* semantic-core errors are not allowed to shrink a proven native card;
* one-off larger cards are accepted only when their edges align with the
  repeated page structure.
"""

from dataclasses import dataclass
import math
import statistics

from ..domain import BBox, OfferCandidate, PageScene, SemanticRole, VisualObject


@dataclass(frozen=True)
class NativePanel:
    bbox: BBox
    confidence: float
    source_id: str
    source_type: str = "CONTAINER"
    repetition_score: float = 0.0


def _same_geometry(first: BBox, second: BBox) -> bool:
    if min(first.width, first.height, second.width, second.height) <= 0:
        return False
    width_ratio = min(first.width, second.width) / max(first.width, second.width)
    height_ratio = min(first.height, second.height) / max(first.height, second.height)
    area_ratio = min(first.area, second.area) / max(first.area, second.area)
    return width_ratio >= .90 and height_ratio >= .90 and area_ratio >= .84


def _container_candidates(page: PageScene) -> list[VisualObject]:
    """Return deduplicated, card-sized VECTOR containers only."""
    page_area = max(1.0, page.width * page.height)
    raw = []
    for obj in page.objects:
        if obj.semantic_role != SemanticRole.CONTAINER:
            continue
        fraction = obj.bbox.area / page_area
        aspect = max(
            obj.bbox.width / max(1.0, obj.bbox.height),
            obj.bbox.height / max(1.0, obj.bbox.width),
        )
        if not (.006 <= fraction <= .50):
            continue
        if aspect > 5.5:
            continue
        raw.append(obj)

    deduped: list[VisualObject] = []
    for obj in sorted(raw, key=lambda item: item.bbox.area):
        duplicate = next(
            (
                other
                for other in deduped
                if abs(other.bbox.x0 - obj.bbox.x0) <= 1.8
                and abs(other.bbox.top - obj.bbox.top) <= 1.8
                and abs(other.bbox.x1 - obj.bbox.x1) <= 1.8
                and abs(other.bbox.bottom - obj.bbox.bottom) <= 1.8
            ),
            None,
        )
        if duplicate is None:
            deduped.append(obj)
    return deduped


def _geometry_families(containers: list[VisualObject]) -> list[list[VisualObject]]:
    """Connected components of containers with repeated width/height."""
    remaining = list(containers)
    families: list[list[VisualObject]] = []
    while remaining:
        seed = remaining.pop(0)
        family = [seed]
        changed = True
        while changed:
            changed = False
            for obj in list(remaining):
                if any(_same_geometry(obj.bbox, member.bbox) for member in family):
                    family.append(obj)
                    remaining.remove(obj)
                    changed = True
        families.append(family)
    return families


def _spreads_over_page(family: list[VisualObject]) -> bool:
    if len(family) < 2:
        return False
    median_w = statistics.median(obj.bbox.width for obj in family)
    median_h = statistics.median(obj.bbox.height for obj in family)
    xs = [obj.bbox.cx for obj in family]
    ys = [obj.bbox.cy for obj in family]
    return (
        max(xs) - min(xs) >= median_w * .75
        or max(ys) - min(ys) >= median_h * .75
    )


def _qualifying_families(
    page: PageScene,
    containers: list[VisualObject],
    price_count: int,
) -> list[list[VisualObject]]:
    page_area = max(1.0, page.width * page.height)
    families = []
    for family in _geometry_families(containers):
        coverage = sum(obj.bbox.area for obj in family) / page_area
        regular_grid = len(family) >= 4 and coverage >= .18 and _spreads_over_page(family)
        simple_page = (
            2 <= len(family) <= 3
            and len(containers) <= 4
            and price_count <= len(family)
            and coverage >= .40
            and _spreads_over_page(family)
        )
        if regular_grid or simple_page:
            families.append(family)
    return families


def _near(value: float, references: list[float], tolerance: float) -> bool:
    return any(abs(value - reference) <= tolerance for reference in references)


def _aligned_to_structure(box: BBox, reference: list[VisualObject], tolerance: float) -> bool:
    """Allow a one-off spanning card only when all four edges follow the grid."""
    x_edges = [value for obj in reference for value in (obj.bbox.x0, obj.bbox.x1)]
    y_edges = [value for obj in reference for value in (obj.bbox.top, obj.bbox.bottom)]
    return (
        _near(box.x0, x_edges, tolerance)
        and _near(box.x1, x_edges, tolerance)
        and _near(box.top, y_edges, tolerance)
        and _near(box.bottom, y_edges, tolerance)
    )


def _crosses_header_footer(page: PageScene, box: BBox) -> bool:
    for obj in page.objects:
        if obj.semantic_role != SemanticRole.HEADER_FOOTER:
            continue
        overlap = box.intersection_area(obj.bbox)
        if overlap <= 0:
            continue
        ratio = overlap / max(1.0, min(box.area, obj.bbox.area))
        if ratio >= .25:
            return True
    return False


def _structural_panels(page: PageScene, price_boxes: list[BBox]) -> list[NativePanel]:
    containers = _container_candidates(page)
    families = _qualifying_families(page, containers, len(price_boxes))
    if not families:
        return []

    reference = [obj for family in families for obj in family]
    family_ids = {obj.id for obj in reference}
    base_area = statistics.median(obj.bbox.area for obj in reference)
    base_scale = math.sqrt(max(1.0, base_area))
    tolerance = max(2.5, base_scale * .04)

    panels: list[NativePanel] = []
    for obj in containers:
        box = obj.bbox
        ratio = box.area / max(1.0, base_area)
        if not (.50 <= ratio <= 5.0):
            continue
        if _crosses_header_footer(page, box):
            continue

        contained_prices = [
            price for price in price_boxes
            if box.contains_point(price.cx, price.cy, 1.5)
        ]
        if len(contained_prices) != 1:
            continue

        repeated = obj.id in family_ids
        if not repeated and not _aligned_to_structure(box, reference, tolerance):
            continue

        panels.append(
            NativePanel(
                bbox=box,
                confidence=.995 if repeated else .93,
                source_id=obj.id,
                repetition_score=1.0 if repeated else .55,
            )
        )

    unique: list[NativePanel] = []
    for panel in sorted(panels, key=lambda item: item.bbox.area):
        if any(
            abs(other.bbox.x0 - panel.bbox.x0) <= 1.8
            and abs(other.bbox.top - panel.bbox.top) <= 1.8
            and abs(other.bbox.x1 - panel.bbox.x1) <= 1.8
            and abs(other.bbox.bottom - panel.bbox.bottom) <= 1.8
            for other in unique
        ):
            continue
        unique.append(panel)
    return unique


def detect_native_panels(
    page: PageScene,
    candidates: list[OfferCandidate],
    nuclei: dict[str, BBox],
    main_prices: dict[str, BBox],
) -> dict[str, NativePanel]:
    """Map priced offers to exact vector cards.

    ``nuclei`` is intentionally ignored for PANEL_NATIVE detection. A semantic
    mistake must never shrink a card whose vector geometry and price ownership
    are already proven.
    """
    del nuclei

    panels = _structural_panels(page, list(main_prices.values()))
    if not panels:
        return {}

    result: dict[str, NativePanel] = {}
    for candidate in candidates:
        own_price = main_prices.get(candidate.id)
        if own_price is None:
            continue

        matches = [
            panel for panel in panels
            if panel.bbox.contains_point(own_price.cx, own_price.cy, 1.5)
        ]
        if matches:
            result[candidate.id] = min(matches, key=lambda panel: panel.bbox.area)

    return result
'''


def patch_classifier() -> None:
    path = CLASSIFIER
    text = path.read_text(encoding="utf-8")
    old = '''TECHNICAL_ONLY = re.compile(
    r"^\\s*(?:\\d+\\s*(?:mois|ans?)\\s*)+$|"
    r"^\\s*(?:\\d+\\s*(?:couverts?|programmes?)\\s*)+$|"
    r"^\\s*chaud\\s*/\\s*froid\\s*$",
    re.I,
)'''
    new = '''TECHNICAL_ONLY = re.compile(
    r"^\\s*(?:\\d+\\s*(?:mois|ans?)\\s*)+$|"
    r"^\\s*(?:\\d+\\s*(?:couverts?|programmes?)\\s*)+$|"
    r"^\\s*\\d+(?:[,.]\\d+)?\\s*feux?\\s*$|"
    r"^\\s*chaud\\s*/\\s*froid\\s*$",
    re.I,
)'''
    text = replace_once(text, old, new, "classifier TECHNICAL_ONLY")
    path.write_text(text, encoding="utf-8")


def patch_region_solver() -> None:
    path = REGION
    text = path.read_text(encoding="utf-8")

    old = '''            if proposed_first.contains(first.semantic_core): first.region = proposed_first
            if proposed_second.contains(second.semantic_core): second.region = proposed_second'''
    new = '''            # A proven native PDF card is an absolute boundary. Conflict
            # reconciliation may shrink FREE_LAYOUT regions, never the card.
            if first.mode != "panel_native" and proposed_first.contains(first.semantic_core):
                first.region = proposed_first
            if second.mode != "panel_native" and proposed_second.contains(second.semantic_core):
                second.region = proposed_second'''
    text = replace_once(text, old, new, "region conflict native lock")

    old = '''    for offer_id, solution in solutions.items():
        region = solution.region
        foreign_points = ['''
    new = '''    for offer_id, solution in solutions.items():
        # Native structured cards were proven from vector geometry + unique
        # PRICE_MAIN ownership. Semantic mistakes in neighbouring candidates
        # must never clip these exact PDF boundaries afterwards.
        if solution.mode == "panel_native":
            solution.region = solution.region.clip(page.width, page.height)
            continue
        region = solution.region
        foreign_points = ['''
    text = replace_once(text, old, new, "region exclusivity native lock")

    path.write_text(text, encoding="utf-8")


RESOLVER_HELPERS = r'''
def _main_price_boxes_for_candidates(page: PageScene, candidates: list[OfferCandidate]) -> dict[str, BBox]:
    facts = {fact.id: fact for fact in page.numeric_facts}
    result: dict[str, BBox] = {}
    for candidate in candidates:
        main = next(
            (
                facts[fact_id]
                for fact_id in candidate.numeric_ids
                if fact_id in facts and facts[fact_id].role == NumericRole.PRICE_MAIN
            ),
            None,
        )
        if main is not None:
            result[candidate.id] = main.bbox
    return result


def _suppress_unpriced_inside_native_panels(
    page: PageScene,
    candidates: list[OfferCandidate],
    native_panels: dict[str, NativePanel],
) -> None:
    """One priced native card must never spawn a second unpriced OFFER."""
    if not native_panels:
        return
    facts = {fact.id: fact for fact in page.numeric_facts}
    panel_boxes = [panel.bbox for panel in native_panels.values()]
    kept = []
    for candidate in candidates:
        has_main = any(
            fact_id in facts and facts[fact_id].role == NumericRole.PRICE_MAIN
            for fact_id in candidate.numeric_ids
        )
        if has_main or candidate.bbox is None:
            kept.append(candidate)
            continue
        inside_priced_card = any(
            panel.contains_point(candidate.bbox.cx, candidate.bbox.cy, 1.0)
            for panel in panel_boxes
        )
        if not inside_priced_card:
            kept.append(candidate)
    candidates[:] = kept


def _lock_native_panel_contents(
    page: PageScene,
    candidates: list[OfferCandidate],
    native_panels: dict[str, NativePanel],
) -> None:
    """Make exact native-card containment stronger than graph proximity."""
    if not native_panels:
        return

    by_id = {candidate.id: candidate for candidate in candidates}
    eligible_roles = {
        SemanticRole.PRODUCT_TEXT,
        SemanticRole.BRAND,
        SemanticRole.ARABIC_TEXT,
        SemanticRole.QUANTITY,
        SemanticRole.MODEL,
        SemanticRole.PROMOTION,
        SemanticRole.PRICE_BASIS,
        SemanticRole.TECHNICAL_SPEC,
    }

    protected_object_ids = {
        obj.id
        for obj in page.objects
        if obj.raw_type in {"line", "image"}
        and any(
            panel.bbox.contains_point(obj.bbox.cx, obj.bbox.cy, 1.0)
            for panel in native_panels.values()
        )
    }
    protected_secondary_ids = {
        fact.id
        for fact in page.numeric_facts
        if fact.role != NumericRole.PRICE_MAIN
        and any(
            panel.bbox.contains_point(fact.bbox.cx, fact.bbox.cy, 1.0)
            for panel in native_panels.values()
        )
    }

    for candidate in candidates:
        candidate.object_ids = [
            obj_id for obj_id in candidate.object_ids
            if obj_id not in protected_object_ids
        ]
        candidate.numeric_ids = [
            fact_id for fact_id in candidate.numeric_ids
            if fact_id not in protected_secondary_ids
        ]

    for candidate_id, panel in native_panels.items():
        candidate = by_id.get(candidate_id)
        if candidate is None:
            continue
        box = panel.bbox

        for obj in page.objects:
            if not box.contains_point(obj.bbox.cx, obj.bbox.cy, 1.0):
                continue
            if obj.raw_type == "line" and obj.semantic_role in eligible_roles:
                if obj.id not in candidate.object_ids:
                    candidate.object_ids.append(obj.id)
                candidate.assignments[obj.id] = 1.0
            elif (
                obj.semantic_role == SemanticRole.IMAGE
                and obj.bbox.area <= box.area * .92
                and obj.id not in candidate.object_ids
            ):
                candidate.object_ids.append(obj.id)
                candidate.assignments[obj.id] = .95

        for fact in page.numeric_facts:
            if fact.role == NumericRole.PRICE_MAIN:
                continue
            if box.contains_point(fact.bbox.cx, fact.bbox.cy, 1.0):
                if fact.id not in candidate.numeric_ids:
                    candidate.numeric_ids.append(fact.id)


'''


def patch_resolver() -> None:
    path = RESOLVER
    text = path.read_text(encoding="utf-8")

    old = '''from ..semantics import parse_promotion
from .region_solver import RegionSolution, infer_safe_regions, solve_page_regions
'''
    new = '''from ..semantics import parse_promotion
from .panel_detector import NativePanel, detect_native_panels
from .region_solver import RegionSolution, infer_safe_regions, solve_page_regions
'''
    text = replace_once(text, old, new, "resolver import panel_detector")

    marker = '''def _assign_objects(page: PageScene, graph: SpatialGraph, candidates: list[OfferCandidate]) -> None:
'''
    text = replace_once(
        text,
        marker,
        RESOLVER_HELPERS + marker,
        "resolver insert card-first helpers",
    )

    old = '''    for page in document.pages:
        graph = build_spatial_graph(page, document.style)
        candidates = _offer_candidates(page, graph)
        _assign_objects(page, graph, candidates)
        _merge_product_with_priced_brand(page, candidates)
        _reassign_secondary_facts(page, candidates)
        region_solutions = solve_page_regions(page, candidates)
        page_offers = _assemble(page, candidates, graph, document.style, region_solutions)
'''
    new = '''    for page in document.pages:
        graph = build_spatial_graph(page, document.style)
        candidates = _offer_candidates(page, graph)

        # CARD FIRST: discover exact vector cards from geometry + PRICE_MAIN
        # ownership before graph-based semantic assignment.
        main_price_boxes = _main_price_boxes_for_candidates(page, candidates)
        native_panels = detect_native_panels(page, candidates, {}, main_price_boxes)
        _suppress_unpriced_inside_native_panels(page, candidates, native_panels)

        _assign_objects(page, graph, candidates)
        _merge_product_with_priced_brand(page, candidates)
        _reassign_secondary_facts(page, candidates)

        # Rebuild structured-card ownership by physical containment after all
        # graph refinements. No neighbouring cell can leak back in afterwards.
        _lock_native_panel_contents(page, candidates, native_panels)

        region_solutions = solve_page_regions(page, candidates)
        page_offers = _assemble(page, candidates, graph, document.style, region_solutions)
'''
    text = replace_once(text, old, new, "resolver card-first pipeline")

    path.write_text(text, encoding="utf-8")


def main() -> None:
    missing = [str(path) for path in FILES if not path.exists()]
    if missing:
        raise SystemExit("Fichiers manquants:\n" + "\n".join(missing))

    for path in FILES:
        backup(path)

    PANEL.write_text(PANEL_CONTENT, encoding="utf-8")
    patch_classifier()
    patch_region_solver()
    patch_resolver()

    print("CARD-FIRST FIX APPLIQUE")
    print("Fichiers modifies:")
    for path in FILES:
        print(" -", path.relative_to(ROOT))
    print()
    print("Backups .bak_card_first crees si absents.")
    print(r"Lance maintenant: .\test.ps1")


if __name__ == "__main__":
    try:
        main()
    except Exception as exc:
        print(f"ERREUR: {exc}", file=sys.stderr)
        raise
