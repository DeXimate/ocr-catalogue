...
function initColumnResize(){
  ...
  const saved=JSON.parse(localStorage.getItem(columnWidthKey)||'null');
  const measured=headers.map(header=>Math.round(header.getBoundingClientRect().width));
  const widths=headers.map((header,index)=>{
    const minimum=index===0?36:(index===1?70:65);
    return Math.max(minimum,Number(saved?.[index])||measured[index]);
  });
  ...
  headers.forEach((header,index)=>{
    ...
    const setWidth=value=>{
      const minimum=index===1?70:65;
      widths[index]=Math.max(minimum,Math.round(value));
      col.style.width=`${widths[index]}px`;
      const total=widths.reduce((sum,width)=>sum+width,0);
      table.style.width=`${total}px`;
      table.style.minWidth=`${total}px`;
    };
    const saveWidths=()=>localStorage.setItem(columnWidthKey,JSON.stringify(widths));
