...
def replace_function(text: str, name: str, next_name: str, replacement: str) -> str:
    """Replace a top-level function up to the next function OR class.

    app.py has:
        def resume_incomplete_jobs(...)
        class Handler(...)
    so the previous patch failed because it searched only for `def Handler(`.
    """
    ...
    for end_marker in (
        ...
        position = text.find(end_marker, start)
        if position >= 0:
            candidates.append(position)

    if not candidates:
        raise RuntimeError(
            f"bloc suivant {next_name} introuvable après {name}"
        )
...
def finalize_cancelled_job(job_id: str) -> dict:
    ...
    with _lock:
        ...
        if metadata.is_file():
            ...
            # Hide it immediately from the catalogue list, then remove the
            # complete job tree in the background once the worker has stopped.
...
FINALIZE_DELETE = r'''def finalize_cancelled_job(job_id: str) -> dict:
    """Permanently delete the cancelled catalogue from the application."""
    folder = job_folder(job_id)
    with _lock:
        metadata = folder / "job.json"
...
def patch_storage() -> None:
    ...
    if "class JobCancelled(" not in text:
        ...
        # V2 may already have been partially applied. Replace its old
        # "preserve source" finalizer with the new delete-everything behavior.
...
$('#cancelProcessing').onclick=async()=>{
  ...
  const confirmed=window.confirm(
    `Annuler le traitement de "${filename}" ?\n\nLe catalogue importé sera supprimé de l’application avec toutes ses pages, images, crops, résultats partiels et métadonnées.\n\nLe fichier original dans Windows/OneDrive ne sera pas supprimé.`
  );
  if(!confirmed)return;
  ...
  try{
    ...
    toast('Annulation demandée — suppression du catalogue en cours');
    ...
    for(let attempt=0;attempt<1200;attempt++){
      try{
        ...
        if(!String(error.message||'').includes('Suppression introuvable'))throw error;
      }
      await wait(500);
    }
    if(deletion?.status!=='deleted')throw new Error('La suppression complète prend trop de temps');

    if(localStorage.getItem('ocr_catalogue_selected_job')===id){
      localStorage.removeItem('ocr_catalogue_selected_job');
    }
    state.job=null;
    state.products=[];
    setDirty(false);
    await loadJobs();
    render();
    toast('Traitement annulé — catalogue supprimé');
  }catch(error){
