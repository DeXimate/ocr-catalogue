...
PROMOTION = re.compile(r"\b(?:OFFRE|GRATUIT(?:E|ES|S)?|DONT\s+\d+|\d+\s*\+\s*\d+|SUR\s+LE\s+\d|VERS[ÉE]S?)\b|^\s*\+", re.I)
...
def _price_role(page: PageScene, bbox: BBox) -> tuple[NumericRole, float, list[str]]:
    ...
    if _preceded_by_reference_cue(page, bbox):
        return NumericRole.PRICE_REFERENCE, .94, ["marqueur_prix_reference"]
...
def _find_prices(page: PageScene) -> list[NumericFact]:
    ...
    role_priority = {NumericRole.PRICE_REFERENCE: 3, NumericRole.CASHBACK: 3, NumericRole.CREDIT_PAYMENT: 3, NumericRole.PRICE_MAIN: 1}
...
def parse_promotion(objects: list[VisualObject], cashback: str = "") -> str:
    if cashback:
        return cashback
    lines = [obj.text.replace("ERFFO", "OFFRE") for obj in sorted(objects, key=lambda item: (item.bbox.cy, item.bbox.x0)) if obj.semantic_role == SemanticRole.PROMOTION]
    joined = " ".join(lines)
    if not joined:
        return ""
    plus_item = re.search(r"\+\s*([A-Za-zÀ-ÿ][A-Za-zÀ-ÿ\s]{1,50}?)\s+GRATUIT(ES?|S)?\b", joined, re.I)
    if plus_item:
        suffix = "gratuit" + (plus_item.group(2) or "").lower()
        return f"+ {re.sub(r'\s+', ' ', plus_item.group(1)).strip().capitalize()} {suffix}"
    plus = re.search(r"(\d+)\s*\+\s*(\d+)", joined)
    if plus and re.search(r"GRATUIT", joined, re.I):
